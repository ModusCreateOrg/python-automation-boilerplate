...
from tests.utils import get_testrail_params
...
class API(object):
    ...
    def _conf(self):
        ...
        _email = os.environ.get(tr_email) or config.get('user_email')
        _key = os.environ.get(tr_key) or config.get('user_key')
        _url = os.environ.get(tr_url) or config.get('url')
        ...
        elif config.get('verify_ssl') is not None:
            verify_ssl = config.get('verify_ssl').lower() == 'true'
        else:
            verify_ssl = True
    ...
    def add_case(self, case):
        ...
        self.flush_cache()
        return self._post('add_case/%s' % section_id, case)

    def update_case(self, case):
        case_id = case.get('id')
        # TODO get update cache working for now reset cache
        self.flush_cache()
        return self._post('update_case/%s' % case_id, case)
    ...
    # Template Requests
    def templates(self):
        if self._refresh(self._templates['ts']):
            # get new value, if request is good update value with new ts.
            _templates = self._get('get_templates/%s' % self._project_id)
            self._templates['value'] = _templates
            self._templates['ts'] = datetime.now()
        return self._templates['value']
    ...
    def add_results_for_cases(self, results, run_id):
        response = self._post('add_results_for_cases/%s' % run_id, results)

...
def get_env_name(driver_name: str, caps: dict):
    ...
    elif driver_name == ['Appium', 'Custom_Driver', 'BrowserStack_app']:
        os_name = caps['deviceName'] if 'deviceName' in caps else caps['device']
        os_version = '_%s' % caps['os_version'] if 'os_version' in caps else caps['platformVersion']
        browser_name = '-app'
    else:
        import platform
